  ...
  return (
    <div className="min-h-screen bg-[var(--bg-page)]">
      ...
      <section className="pt-32 pb-16 px-6 text-center">
        <h1 className="text-4xl sm:text-5xl md:text-6xl font-display font-medium text-[var(--text-primary)] tracking-tight mb-6">
          Choose Your Occasion
        </h1>
        <p className="text-lg text-[var(--text-secondary)] font-light max-w-xl mx-auto leading-relaxed">
          ...
          {CATEGORIES.map((cat) => {
            if (cat.comingSoon) {
              return (
                <div
                  key={cat.slug}
                  className="relative bg-[var(--bg-surface)] rounded-2xl p-8 border border-[var(--border-subtle)] opacity-55 cursor-default transition-all duration-300"
                >
                  <div
                    className="absolute top-0 left-0 right-0 h-1 rounded-t-2xl opacity-40"
                    style={{ background: cat.color }}
                  />
                  <h3 className="text-lg font-display font-medium text-[var(--text-secondary)] mb-2">
                    {cat.name}
                  </h3>
            ...
            return (
              <Link
                key={cat.slug}
                to={`/shop/${cat.slug}`}
                className="group relative bg-[var(--bg-surface)] rounded-2xl p-8 border border-[var(--border-subtle)] hover:border-[var(--accent-gold-border-hover)] hover:bg-[var(--bg-surface-hover)] hover:shadow-lg hover:shadow-[var(--accent-gold-border)] transition-all duration-300"
              >
                <div
                  className="absolute top-0 left-0 right-0 h-1 rounded-t-2xl"
                  style={{ background: cat.color }}
                />
                <h3 className="text-lg font-display font-medium text-[var(--text-primary)] mb-2 group-hover:text-[var(--accent-gold)] transition-colors">
                  {cat.name}
                </h3>
                <p className="text-sm text-[var(--text-secondary)] font-light leading-relaxed">
                  {cat.tagline}
                </p>
                <div className="mt-6 flex items-center text-[11px] uppercase tracking-[0.15em] text-[var(--text-muted)] group-hover:text-[var(--accent-gold)] transition-colors font-medium">
                  <span>Explore</span>
